...
Set ws = wb.Worksheets("MarieGaillard")
wb.Activate
...
Sub load_data()
Dim derligne, derlignelwb, derlignedate As Integer
...
sfdc.Activate
Sheets(1).Activate
Range(Cells(2, 31), Cells(derligne, 31)).Copy
wb.Activate
ws.Cells(derlignelwb + 1, 3).Select
ActiveCell.PasteSpecial Paste:=xlPasteValues

'CC CV PRICE INCREASE'
sfdc.Activate
Sheets(1).Activate
Range(Cells(2, 26), Cells(derligne, 26)).Copy
wb.Activate
ws.Cells(derlignelwb + 1, 11).Select
ActiveCell.PasteSpecial Paste:=xlPasteValues

'CC CV SAAS'
sfdc.Activate
Sheets(1).Activate
Range(Cells(2, 26), Cells(derligne, 28)).Copy
wb.Activate
ws.Cells(derlignelwb + 1, 12).Select
ActiveCell.PasteSpecial Paste:=xlPasteValues
...
'CC  Souscription term '
sfdc.Activate
Sheets(1).Activate
Range(Cells(2, 16), Cells(derligne, 16)).Copy
wb.Activate
ws.Cells(derlignelwb + 1, 10).Select
ActiveCell.PasteSpecial Paste:=xlPasteValues

'Mise en forme'
moisco = Application.InputBox("De quel mois commercial s'agit-il? (M01, M02...)")
...
Dim dico
Dim mois As String
Dim taux59, taux79, taux100, taux100max, saas, psorate, rule1, rule2, rule3, rule4, rule5, rule6 As Currency
...
'calcul de la rule of X
For i = 10 To lastrow(2)
    Cells(i, 19) = Cells(i, 10) + Cells(i, 11) * 100
...
Dim palier, ponderation, coeff As Double
...
Cells(ligne, 7) = ponderation * licence
